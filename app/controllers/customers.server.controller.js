...
exports.customerByName = function(req, res, next, name) {
	Customer.find({'name': 'ninini'}).exec(function(err, customer) {
		if (err) return next(err);
		if (! customer) return next(new Error('Failed to load Customer '));
		res.jsonp(customer);
		next();
	});
};
